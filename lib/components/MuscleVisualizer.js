...
import { useGLTF, Center, OrbitControls } from '@react-three/drei/native'
...
// Rigify deform bones carry an .L/.R suffix (GLTF strips the dot → trailing L/R).
...
const pickGroupFromHit = (mesh, face, worldPoint) => {
  ...
  // Skinned mesh (male rig): read the dominant skin bone at the hit face's first
  // vertex, then map that bone to its muscle group.
  ...
  // Unrigged mesh (female base): classify the local hit position into a region
  // band, matching paintRegionMesh's axis logic.
  const geo = mesh.geometry
...
function HumanBodyModel({ model, viewMode, targetedMuscles, groupColors, girthScale = 1, heightScale = 1, onReady, onPick }) {
  const groupRef   = useRef(null)
  const readyFired = useRef(false)

  // Tap → raycast hit → body-part group. r3f's onClick only fires on a tap that
  // didn't drag (drags are consumed by OrbitControls), so this never conflicts
  // with rotate/pan. e.object is the actual mesh hit; e.face / e.point locate it.
  const handleClick = (e) => {
    if (!onPick) return
    e.stopPropagation()
      ...
      <View style={styles.viewer}>
        ...
        <ModelErrorBoundary>
          ...
          >
            ...
            {/* Free camera — one-finger drag rotates, pinch zooms, two-finger pans.
        ...
        {/* Recenter — resets the free camera to its default framing (shows even
            when the preset buttons are hidden, since pan/zoom can lose the body). */}
        {controls && (
          <TouchableOpacity
