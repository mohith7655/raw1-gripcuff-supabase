...
export function EditTweetModal({ visible, post, onClose, onSaved }: EditTweetModalProps) {
  ...
  }, [saving, slideAnim, onClose]);

  const hasChanged = text.trim() !== (post?.caption ?? '').trim();
  const canSave = hasChanged && text.trim().length > 0 && !saving;
  ...
  const handleSave = useCallback(async () => {
    ...
    setError(null);
    try {
      const updated = await FeedService.updatePostCaption(post.id, text.trim());
  ...
  }, [canSave, post, text, mentionedUserIds, onSaved, handleClose]);
      ...
      <Animated.View style={[styles.sheet, { transform: [{ translateY: slideAnim }] }]}>
        ...
        >
          ...
          <View style={styles.header}>
            <TouchableOpacity onPress={handleClose} disabled={saving} style={styles.closeBtn} activeOpacity={0.7}>
              <X size={20} color={TEXT_SECONDARY} />
            </TouchableOpacity>
            <Text style={styles.headerTitle}>Edit Tweet</Text>
            <TouchableOpacity
              onPress={handleSave}
              disabled={!canSave}
