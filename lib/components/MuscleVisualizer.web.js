...
// three's own OrbitControls (same three/examples family as GLTFLoader above).
// We avoid @react-three/drei on web — its ESM entry uses `import.meta`, which the
// Expo/Hermes web bundler can't parse ("Cannot use 'import.meta' outside a module").
...
// ─── Raycast hit → body-part group ────────────────────────────────────────────
// Turns a 3D tap on the mesh into a { group, side } using the SAME bone / region
// maps used for painting, so selecting a part works at any camera angle or zoom.
const sideFromBone = (raw) => {
  const sm = /([LR])\d*$/.exec(raw || '')
  return sm ? (sm[1] === 'L' ? 'left' : 'right') : null
}

const pickGroupFromHit = (mesh, face, worldPoint) => {
  if (!mesh) return null

  // Skinned mesh (male rig): dominant skin bone at the hit face → muscle group.
  if (mesh.isSkinnedMesh && face) {
    const geo        = mesh.geometry
    const skinIndex  = geo?.attributes?.skinIndex
    const skinWeight = geo?.attributes?.skinWeight
    const bones      = mesh.skeleton?.bones || []
    ...
    const vi = face.a
    let bestW = -1, bestBone = -1
    for (let k = 0; k < 4; k++) {
      const w = skinWeight.getComponent(vi, k)
      if (w > bestW) { bestW = w; bestBone = skinIndex.getComponent(vi, k) }
    }
    const bone = bones[bestBone]
    if (!bone) return null
    const n = normName(bone.name)
    const side = sideFromBone(bone.name)
    for (const bg of BONE_GROUPS) {
      if (bg.frags.some((f) => n.includes(normName(f)))) return { group: bg.group, side }
    }
    return null
  }

  // Unrigged mesh (female base): classify the local hit position into a region band.
  const geo = mesh.geometry
  if (!geo || !worldPoint) return null
  if (!geo.boundingBox) geo.computeBoundingBox()
  const bb = geo.boundingBox
  ...
  const wHalf   = Math.max(1e-6, (bb.max[wAxis] - bb.min[wAxis]) / 2)
  const hMaxEnd = bb.max[hAxis]
  const t  = (hMaxEnd - local[hAxis]) / hRange
  const wn = (local[wAxis] - wCenter) / wHalf
  const side = wn > 0 ? 'left' : 'right'
  for (const r of REGION_BANDS) {
    if (t < r.tMin || t > r.tMax) continue
    if (r.xMin != null && Math.abs(wn) < r.xMin) continue
    return { group: r.group, side }
  }
  return null
}
...
function HumanBodyModel({ modelUri, viewMode, targetedMuscles = [], groupColors, girthScale = 1, heightScale = 1, onReady, onPick }) {
  const groupRef   = useRef(null)
  const readyFired = useRef(false)

  // Tap → raycast hit → body-part group. r3f's onClick fires only on a tap that
  // didn't drag (drags go to OrbitControls), so selecting never fights rotate/pan.
  const handleClick = (e) => {
    if (!onPick) return
    e.stopPropagation()
    const res = pickGroupFromHit(e.object, e.face, e.point)
    if (res) onPick(res.group, res.side)
  }
...
// ─── Free-camera controls (drag rotate · scroll/pinch zoom · right-drag pan) ───
// Thin R3F wrapper around three's OrbitControls, updated each frame for damping.
// controlsRef is populated so the recenter button can call `.reset()`.
function CameraControls({ controlsRef, minDistance = 2.6, maxDistance = 11 }) {
  ...
  }, [controls, minDistance, maxDistance, controlsRef])
...
export default function MuscleVisualizer({ gender, targetedMuscles = [], groupColors, activeView, view, onViewChange, height, overlay, hideControls, girthScale = 1, heightScale = 1, onPartSelect, controls = true }) {
      ...
      <View style={styles.viewer}>
        ...
        <ModelErrorBoundary>
          ...
          >
            ...
            {/* Free camera — drag rotates, scroll/pinch zooms, right-drag pans.
                Distance limits keep the body from vanishing or clipping through. */}
            {controls && (
              <CameraControls controlsRef={controlsRef} minDistance={2.6} maxDistance={11} />
            )}
          </Canvas>
        </ModelErrorBoundary>
        ...
        {/* Caller-supplied overlay (decorative labels / markers only — selection
            now happens via a raycast on the canvas). It must stay fully
            pointer-transparent: on react-native-web a "box-none" layer over the
            <canvas> still swallows drags, blocking OrbitControls rotate/zoom/pan. */}
        {overlay ? (
          <View style={StyleSheet.absoluteFill} pointerEvents="none">{overlay}</View>
        ) : null}

        {/* Recenter — resets the free camera (pan/zoom can lose the body). */}
        {controls && (
          <TouchableOpacity
