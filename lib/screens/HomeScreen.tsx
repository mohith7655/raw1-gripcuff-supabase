          ...
          {userLoading ? (
            ...
            <>
                ...
                >
                  <View style={{ alignItems: 'center', gap: 6 }}>
                    ...
                    {/* Upgrade link right below the tier level number — opens the
                        Gripcuff membership comparison. */}
                    <TouchableOpacity
                      onPress={() => setShowTiersModal(true)}
                      activeOpacity={0.7}
                      hitSlop={{ top: 6, bottom: 6, left: 10, right: 10 }}
                      {...(Platform.OS === 'web' ? { onClick: (e: any) => { e?.stopPropagation?.(); setShowTiersModal(true); } } : {})}
                    >
              ...
              {(recentlyWatched.length > 0 || totalFavouritesCount > 0) && (() => {
                ...
                const favItems = [
                  ...allVids.filter(v => favExerciseIds.has(v.id)).map(v => ({ id: v.id, title: v.title, videoUrl: v.videoUrl, thumbnail: (v as any).thumbnail, category: (v as any).category, difficulty: (v as any).difficulty })),
                  ...allProgs
                    .filter(p => favWorkoutIds.has(p.id) || p.videos.some(v => favWorkoutIds.has(v.id)))
                    .map(p => {
                      const matched = p.videos.find(v => favWorkoutIds.has(v.id));
                      return { id: matched?.id ?? p.id, title: p.title, videoUrl: p.videos?.[0]?.videoUrl, thumbnail: (p as any).thumbnail, category: getProgramCategoryKey(p.id), difficulty: (matched as any)?.difficulty ?? (p as any).level };
                    }),
                ];
                return (
